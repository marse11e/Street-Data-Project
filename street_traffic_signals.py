...
def plot_graph_with_traffic_lights(city, nodes, edges, num_nodes, num_edges, traffic_lights, output_path):
    """Отображает граф с отмеченными светофорами и сохраняет визуализацию."""
    plt.style.use('dark_background')
    fig, ax = plt.subplots(1, figsize=(10, 10))
    edges.plot(ax=ax, edgecolor='white', linewidth=0.3)
    traffic_lights.plot(ax=ax, color='red', markersize=5, label='Светофоры с интернетом', zorder=5)
...
def main():
    ...
    # Извлечение светофоров
    traffic_lights = nodes[nodes['highway'] == 'traffic_signals']

    output_path = f'static/street_graph_img/traffic_lights/{city}_street_network_with_traffic_lights.png'
    plot_graph_with_traffic_lights(city, nodes, edges, num_nodes, num_edges, traffic_lights, output_path)
